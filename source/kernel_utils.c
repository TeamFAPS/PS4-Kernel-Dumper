...
int kpayload_get_fw_version(struct thread *td, struct kpayload_get_fw_version_args* args) {
    ...
    } else if (!memcmp((char*)(&((uint8_t*)__readmsr(0xC0000082))[-KERN_176_XFAST_SYSCALL]), (char[4]){0x7F, 0x45, 0x4C, 0x46}, 4)) {
        
        kernel_base = &((uint8_t*)__readmsr(0xC0000082))[-KERN_176_XFAST_SYSCALL];
        
        if (!memcmp((char*)(kernel_base+KERN_176_PRINTF), (char[12]){0x55, 0x48, 0x89, 0xE5, 0x53, 0x48, 0x83, 0xEC, 0x58, 0x48, 0x8D, 0x1D}, 12)) {
            fw_version = 0x176;
            copyout = (void *)(kernel_base + KERN_176_COPYOUT);
        }
        
    } else if (!memcmp((char*)(&((uint8_t*)__readmsr(0xC0000082))[-KERN_101_XFAST_SYSCALL]), (char[4]){0x7F, 0x45, 0x4C, 0x46}, 4)) {
        
        kernel_base = &((uint8_t*)__readmsr(0xC0000082))[-KERN_101_XFAST_SYSCALL];
        
        if (!memcmp((char*)(kernel_base+KERN_101_PRINTF), (char[12]){0x55, 0x48, 0x89, 0xE5, 0x53, 0x48, 0x83, 0xEC, 0x58, 0x48, 0x8D, 0x1D}, 12)) {
            fw_version = 0x101;
            copyout = (void *)(kernel_base + KERN_101_COPYOUT);
        }
...
int kpayload_jailbreak(struct thread *td, struct kpayload_jailbreak_args* args) {
    ...
    switch (fw_version) {
        case 0x101: 
            ...
            got_prison0   = (void**)&kernel_ptr[KERN_101_PRISON_0];
            got_rootvnode = (void**)&kernel_ptr[KERN_101_ROOTVNODE];
            break;
        case 0x176: 
            kernel_base   = &((uint8_t*)__readmsr(0xC0000082))[-KERN_176_XFAST_SYSCALL];
            kernel_ptr    = (uint8_t*)kernel_base;
            got_prison0   = (void**)&kernel_ptr[KERN_176_PRISON_0];
            got_rootvnode = (void**)&kernel_ptr[KERN_176_ROOTVNODE];
            break;
        case 0x355: 
            kernel_base   = &((uint8_t*)__readmsr(0xC0000082))[-KERN_355_XFAST_SYSCALL];
            kernel_ptr    = (uint8_t*)kernel_base;
